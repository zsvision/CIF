...
# from models.networks.lstm import LSTMEncoder   # 注释掉原来的 LSTM
from models.networks.mamba import MambaEncoder # 导入我们刚刚写的 Mamba
...
class UttFusionModel(BaseModel):
    ...
    def forward(self):
        ...
        final_embd = []
        if 'A' in self.modality:
            self.feat_A = self.netA(self.acoustic)  # netA：Mamba
        ...
        if 'V' in self.modality:
            self.feat_V = self.netV(self.visual)    # netV：Mamba
